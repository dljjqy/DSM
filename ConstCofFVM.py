...
def force(x, y, center, delta=0.025):
    px, py = center
...
class NLinearProblem(BaseProblem):
    def __init__(self, GridSize, u0, point_center=(0.5, 0.5), area=((0, 0), (1, 1)), mu=0.2, eps = 1e-9):
        super().__init__(N=GridSize, area=area)
        ...
        self.h = (self.right - self.left) / self.GridSize
        if self.u0 is None:
        ...
        xx, yy = np.meshgrid(
            np.arange(self.left + self.h/2, self.right, self.h),
            np.arange(self.bottom + self.h/2, self.top, self.h)
        )
        self.force = force(xx, yy, point_center)
        
    def is_dirichlet(self, i, j, e):
        return self.is_boundary(i, j, e)
...
class UniformFVM():
    ...
    def _alpha_beta(self, edge, K, vec, vec_prev, vec_next):
        c = self.lengths[edge] * K * self.normals[edge]
        # t = self.R @ vec
        ...
        return 0, beta_prev, 0, beta_next
    ...
    def assemble_source_term(self):
        self.b += self.dx * self.dy * self.problem.force.flatten()
    
    def get_A(self, problem=None):
        if not problem is None:
            self.problem = problem 
        self.A = lil_matrix((self.n_faces, self.n_faces), dtype=np.float32)
        self.assemble_stiffness_matrix()
        return self.A.tocsr()
    
    def get_B(self, problem=None):
        if not problem is None:
            self.problem = problem 
        self.b = np.zeros(self.n_faces)
