...
class Trainer(BaseTrainer):
    ...
    def train_step(self, data, layouts, bd_cases, maxiter):
        ...
        self.writer.add_scalar("Train-SubiterError", error, self.train_global_idx)
        self.train_global_idx += 1

        return error, loss_val

    def val_step(self, data, layout, bd_cases, maxiter):
        ...
        val_subiter_loss = self.loss_fn(subiter_ans, pre).item()
        val_real_loss = self.loss_fn(real_ans, pre).item()
        error = monitor(pre, B).item()

        return pre, real_ans, error, val_subiter_loss, val_real_loss
